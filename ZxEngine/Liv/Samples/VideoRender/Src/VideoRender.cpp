...
//! The Fx names
const Char g_strFxName[] = "VideoRender";
...
{
	return (IFxBase*)(new CVideoRender);
}
#ifdef __cplusplus
}
#endif
...
void SDLCALL
CVideoRender::fillerup(void *unused, Uint8 * stream, int len)
{
	Uint8*	waveptr		= NULL;
	int		waveleft	= 0;
...
boost::circular_buffer<ZX_Media_Buffer_t> CVideoRender::_MediaBuffers(3);
...
*//*-----------------------------------------------------------------------------*/
Int32 CVideoRender::GetFxInfo(const FX_DESCRIPTOR** ppFxDescriptor)
{
    if(_pFxDescriptor == NULL) {
        ...
        _pFxDescriptor->FxScope = FX_SCOPE_VIDEO;
    }
...
/*-----------------------------------------------------------------------------*//*!
	...
	Initializes the Fx.
*//*-----------------------------------------------------------------------------*/
Int32 CVideoRender::InitFx(IFx **ppFxComponent)
{
	Int32 hr = FX_ERROR;
...
*//*-----------------------------------------------------------------------------*/
Int32 CVideoRender::UpdateFxParam(const std::string strParameter, FX_PARAMETER FxParameter)
{	
	return FX_OK;  
}
...
*//*-----------------------------------------------------------------------------*/
Int32 CVideoRender::DisplayFxPropertyPage(Pvoid pvWndParent)
{
	return FX_OK;
}
...
// 	SDL_CreateWindow("Video Render Window", 200, 200, 400, 300, 
// 		SDL_WINDOW_MAXIMIZED | SDL_WINDOW_MINIMIZED | SDL_WINDOW_RESIZABLE);

	MakeSDLSpec();

	_Screen = SDL_SetVideoMode(320, 240, 16, SDL_DOUBLEBUF | SDL_SWSURFACE);
	if (_Screen == NULL) {
...
Int32 CVideoRender::RunFx()
{
	...
	/*! Start new thread */
	if(pthread_create (&_Thread, NULL, CVideoRender::ThreadFctWrapper, (void *)this) != 0){
		return FX_ERROR;
	}
...
*//*-----------------------------------------------------------------------------*/
void CVideoRender::FxPinState( IFxPin* pFxPin, FX_PIN_STATE PinState )
{
...
*//*-----------------------------------------------------------------------------*/
Int32 CVideoRender::FxPin(IFxPin* pFxPin, IFxMedia* pIFxMedia, FX_STREAM_STATE StreamState)
{
	Int32 hr = FX_OK;
...
void CVideoRender::MakeSDLSpec()
{
// 	_Spec.callback	= CVideoRender::fillerup;
